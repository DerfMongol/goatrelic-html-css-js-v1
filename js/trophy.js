...
var trophyCase = document.querySelector('.trophies')

Object.keys(sportTrophies).forEach(function(sport, index) {
    if (location.pathname.includes(sport)) {
        sportTrophies[sport].forEach(function(trophy) {
            var drpDwn = newElement('div', 'dropdown-trophy', null, trophyCase)
            var image = newElement('img', null, null, drpDwn)
            image.id = trophy.id
            image.src = 'images/' + trophy.src
            var description = newElement('div', 'description', null, drpDwn)
            newElement('div', 'trophy-title', trophy.title, description)
            var nav = newElement('nav', null, null, description)
            var ul = newElement('ul', null, null, nav)
            trophy.info.forEach(function(bullet) {
